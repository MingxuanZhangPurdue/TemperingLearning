...
class Tempering(L.LightningModule):
    def __init__(
        ...
        self.timesteps = torch.flip(self.noise_scheduler.timesteps, [0]).to(self.device)
    ...
    def training_step_epsilon_prediction(self, batch, t):
        ...
        # if t == self.T-1, the noise is not added
        if t == self.T-1:
            noisy_images = clean_images  
        # if t < self.T-1, the noise is added
        elif t < self.T-1:
            timestep = self.timesteps[self.T - t - 2].to(clean_images.device)
            noisy_images = self.noise_scheduler.add_noise(clean_images, torch.randn_like(clean_images), timestep)
        # if t is not in the range of 0 to self.T-1, raise an error
        else:
        ...
        timestep = self.timesteps[self.T - t - 1].to(clean_images.device)
    ...
    def generate_samples(self, batch_size, t=None):
        ...
        assert t is None or 0 <= t < self.T, "t must be in the range of 0 to self.T-1"

        t = self.T-1 if t is None else t

        timestep = -1 if t == self.T-1 else self.timesteps[t].to(noise.device)
